...
// =====================================================================
// MALI Access Union — app-side client (self-contained for this Next app).
// Canonical shared implementation lives in
// packages/supabase-client/mali.ts — keep the two in sync.
// Env: NEXT_PUBLIC_MALI_SUPABASE_URL / NEXT_PUBLIC_MALI_SUPABASE_ANON_KEY
// (gitignored apps/web/.env.local). Publishable (anon) key only — never
// service_role in the browser.
// =====================================================================

const url = process.env.NEXT_PUBLIC_MALI_SUPABASE_URL!;
const anonKey = process.env.NEXT_PUBLIC_MALI_SUPABASE_ANON_KEY!;

export const mali = createClient(url, anonKey);
...
export async function createMaliGroup(supabase: SupabaseClient, input: { name: string; description?: string | null; currency?: string; is_active?: boolean; created_by: string }): Promise<MaliGroup> {
...
export async function addMaliMember(supabase: SupabaseClient, input: { group_id: string; full_name: string; phone?: string | null; national_id?: string | null; role?: string; auth_uid?: string | null }): Promise<MaliMember> {
  const { data, error } = await supabase
...
export async function openMaliCycle(supabase: SupabaseClient, input: { group_id: string; name: string; saving_type: number; state?: number; contribution: number; currency?: string; frequency?: string; pot_target?: number | null; is_rotating?: boolean; created_by: string }): Promise<MaliCycle> {
  const { data, error } = await supabase
    .from("mali_cycle")
    .insert({
      group_id: input.group_id,
...
export async function recordMaliContribution(supabase: SupabaseClient, input: { cycle_id: string; member_id: string; amount: number; currency?: string; channel_id?: number | null; paid_on?: string; ref_no?: string | null }): Promise<MaliContribution> {
  const { data, error } = await supabase
...
export function subscribeMaliStore(supabase: SupabaseClient, onEvent: (e: MaliRealtimeEvent) => void): RealtimeChannel {
